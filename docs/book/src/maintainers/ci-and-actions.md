...
- **history-guard**: fetches full history and rejects a PR or merge-queue ref
  when `git merge-base origin/master HEAD` is empty, preventing a grafted
  second root from collapsing `git blame` after merge
